...
task('full-deploy', 'deploys the entire Lens Protocol').setAction(async ({}, hre) => {
  ...
  // Recipient
  console.log('\n\t-- Deploying Recipient --');
  const recipient = await deployContract(
    new Recipient__factory(deployer).deploy({nonce: deployerNonce++})
  );
  ...
  console.log('\n\t-- Deploying multiRecipientFeeCollectModule --');
  const multiRecipientFeeCollectModule = await deployContract(
    new MultirecipientFeeCollectModule__factory(deployer).deploy(lensHub.address, moduleGlobals.address, {
      nonce: deployerNonce++,
    })
  ); 
  ...
  await waitForTx(
    lensHub.whitelistCollectModule(multiRecipientFeeCollectModule.address, true, { nonce: governanceNonce++ })
  )
  await waitForTx(
  ...
  const addrs = {
    ...
    'multireciepient collect module': multiRecipientFeeCollectModule.address,
